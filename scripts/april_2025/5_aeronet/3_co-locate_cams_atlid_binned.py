...
aod_cams, x_edge, y_edge, _ = binned_statistic_2d(
    all_lat, all_lon, aod_cams, statistic=mean_or_std, bins=[lat_bins, lon_bins])

aod_aeronet_r = df_final['AOD_355nm']
aeronet_lat   = df_final["Site_Latitude(Degrees)"]
aeronet_lon   = df_final["Site_Longitude(Degrees)"]

mask = ~np.isnan(aod_aeronet_r)
aod_aeronet_r = aod_aeronet_r[mask]
aeronet_lat = aeronet_lat[mask]
aeronet_lon = aeronet_lon[mask]

aod_aeronet_r,x_edge, y_edge, _ = binned_statistic_2d(
    aeronet_lat,aeronet_lon,aod_aeronet_r,statistic=mean_or_std, bins=[lat_bins, lon_bins])

lon_centers = (lon_bins[:-1] + lon_bins[1:]) / 2
lat_centers = (lat_bins[:-1] + lat_bins[1:]) / 2

lon_cents, lat_cents = np.meshgrid(lon_centers,lat_centers)
...
# Plot AOD values
im = ax2.pcolormesh(lon_cents,lat_cents,aod_cams,cmap='plasma',norm=colors.LogNorm(vmin=1e-2, vmax=1),transform=ccrs.PlateCarree())
...
diffs = (aod_cams-aod_aeronet_r).flatten()
mask = ~np.isnan(diffs)
diffs = diffs[mask]
lon_cents = lon_cents.flatten()[mask]
lat_cents = lat_cents.flatten()[mask]
...
plt.tight_layout()
fig.savefig('global_AERONET_CAMS_aod_'+month2+'_2025_binned.jpg',bbox_inches='tight')

#below is fine, only cams seems to be wrong
#-ATLID-------------------------------------------------------------------------
