...
for month,month2,fmonth in zip(months,month2s,fmonths):
    ...
    for i in range(len(df_aer_f)):
        ...
        aer_lat = df_aer_f["lat"].iloc[i]
        aer_lon = df_aer_f["lon"].iloc[i]
        aer_aod = df_aer_f["aeronet_aod"].iloc[i]
        ...
        if np.any(mask):
            for match_idx in np.where(mask)[0]:
                all_matches.append({
                    ...
                    "aeronet_lon": aer_lon,
                    "aeronet_aod": aer_aod,
                    "atlid_time": co_loc_time[match_idx],
                    "atlid_lat": co_loc_lats[match_idx],
                    "atlid_lon": co_loc_lons[match_idx],
                    "atlid_aod": co_loc_aods[match_idx],
                    "time_diff_min": time_diff[match_idx]
                })
