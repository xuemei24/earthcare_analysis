...
forecast_period = '03'
#aero_type = 'SSA'
aero_type = 'Dust_Smoke'
if aero_type == 'SSA':
    aerosol_index = np.array([11]) #sea spray
    cams_v_name = 'sea_salt_extinction_'+str(int(forecast_period))
    f_aero_type = 'sea_salt'
elif aero_type == 'Dust_Smoke':
    aerosol_index = np.array([10,13,14,15,27])
    cams_v_name = 'dust_bc_extinction_'+str(int(forecast_period))
    f_aero_type = 'dust_bc'

cams_dir = '/net/pc200254/nobackup/users/wangxu/cams_data/aerosol_mmr/'+month+'/'
fcams = cams_dir+'temp/'+f_aero_type+'_extinction_coefficient_'+month+'_2025_'+str(int(forecast_period))+'.nc'
if fcams[-4:-3] == '0':
...
cams_extcoe_o = ds[cams_v_name].values
print(cams_extcoe_o.shape)
ds.close()

ds = xr.open_dataset('/net/pc190625/nobackup_1/users/wangxu/cams_data/extinction_355nm_multi/aerosol_extinction_coe_355nm_'+month+'2025_'+str(int(forecast_period))+'.nc')
print(ds['aerext355'].values[0].shape)
ds['aerext355'].values[0] = cams_extcoe_o
...
print('cams_extcoe>0',cams_extcoe[cams_extcoe>0])
...
print('cams_extcoe_clear',cams_extcoe_clear[cams_extcoe_clear>0])
...
fname = "selected_files_Africa_july_2025.txt"
...
for i,filen in enumerate(ebd_files):
    ...
    atlid_extcoe,atlid_lats,atlid_lons,atlid_times,atlid_h,tc_cld,tc_all,err = read_h5.get_ext(filen,aerosol_index)
    ...
    print("ds_clear['latitude']",ds_clear['latitude'])
    print("ds_clear['longitude']",ds_clear['longitude'])
    print("ds_clear['forecast_reference_time']",ds_clear['forecast_reference_time'])
    print("atlid_lats",atlid_lats)
    print("atlid_lons",atlid_lons)
    print("atlid_times",atlid_times)
    ...
    print(cams_interp['aerext355'].values[cams_interp['aerext355'].values>0])
    ...
    atlid_extcoe_noerr = np.where(atlid_extcoe/err>=5,atlid_extcoe,np.nan)

    c1 = axs[0].pcolormesh(atlid_timesp, atlid_h//1000, atlid_extcoe_noerr, cmap=cmap, shading='auto',norm=matplotlib.colors.LogNorm(vmax=vmax,vmin=1e-6))
    ...
    axs[0].set_title(aero_type+' ATLID Extinction 355 nm & TC clouds '+orbit_sequence)
    ...
    c4 = axs[3].pcolormesh(atlid_timesp, atlid_h//1000, atlid_extcoe_noerr/err, cmap=ecplt.colormaps.calipso,vmin=0,vmax=20)
    ...
    plt.tight_layout()
    fig.savefig('slices_compo/'+fname[15:-4]+'_'+aero_type+'_atlid_vs_cams_extinction_'+orbit_sequence+'.jpg')
    ...
    ax1.set_title(aero_type,fontsize=15)

    fig.savefig('slices_compo/'+fname[15:-4]+'_'+aero_type+'_atlid_vs_cams_extinction_'+orbit_sequence+'_correlation_hist.jpg')
    plt.close(fig)
    ...
    fig_name = 'slices_compo/'+fname[15:-4]+'_atlid_orbit_'+orbit_sequence+'.jpg'
