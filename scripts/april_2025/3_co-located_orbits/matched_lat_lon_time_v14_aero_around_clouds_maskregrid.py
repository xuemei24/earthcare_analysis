...
dslwc = xr.open_dataset('/net/pc190625/nobackup_1/users/wangxu/cams_data/lwc/specific_cloud_liquid_water_content_april_2025_'+str(int(forecast_period))+'.nc')
lwc = dslwc['clwc'].values[0,1:]

def around_cloud(cloud,data):
    for i in range(cloud.shape[0]):
        cams_ext_i = data[i,:,:,:]
        mask_lwc = cloud[i,:,:,:]>=0.0001
        dilated_lwc = binary_dilation(mask_lwc, structure=np.ones((11,11,11)))
        surrounding_mask_lwc = dilated_lwc & (~mask_lwc)
        cams_temp = np.full_like(cams_ext_i,np.nan,dtype=float)
        cams_temp[surrounding_mask_lwc] = cams_ext_i[surrounding_mask_lwc]
        data[i] = cams_temp
    return data

cams_extcoe_clear = around_cloud(lwc,cams_extcoe)
...
for i,filen in enumerate(ebd_files):
    ...
    cams_interp_varp = np.array(cams_interp_temp)
    ...
    plt.tight_layout()
    fig.savefig('slices_regions/'+fname[15:-4]+'_atlid_vs_cams_extinction_'+orbit_sequence+'_around_liq_clouds.jpg')
    plt.close(fig)
    ...
    fig.savefig('slices_regions/'+fname[15:-4]+'_atlid_vs_cams_extinction_'+orbit_sequence+'_correlation_hist_around_liq_clouds.jpg')
    plt.close(fig)
