...
month = 'july'
cams_dir = '/net/pc200254/nobackup/users/wangxu/cams_data/aerosol_mmr/'+month+'/TTcal/'
fcams0 = xr.open_dataset(cams_dir+'aod355nm_per_composition'+'_'+month+"_2025.nc")
aod_cams_ssa0 = fcams0['ssdryaod355'].values
aod_cams_dust0 = fcams0['dudryaod355'].values+fcams0['bcdryaod355'].values+fcams0['omdryaod355'].values
#fcams3 = xr.open_dataset('aod355nm_per_composition_3_april_2025.nc')
#aod_cams_ssa3 = fcams3['ssdryaod355'].values
#fcams6 = xr.open_dataset('aod355nm_per_composition_6_april_2025.nc')
#aod_cams_ssa6 = fcams6['ssdryaod355'].values
#fcams9 = xr.open_dataset('aod355nm_per_composition_9_april_2025.nc')
#aod_cams_ssa9 = fcams9['ssdryaod355'].values

#stacked_ssa = np.stack([aod_cams_ssa0,aod_cams_ssa3,aod_cams_ssa6,aod_cams_ssa9],axis=0)
#aod_cams_ssa = np.nanmean(np.nanmean(stacked_ssa,axis=0),axis=0)
#stacked_dust = np.stack([aod_cams_dust0,aod_cams_dust3,aod_cams_dust6,aod_cams_dust9],axis=0)
#aod_cams_dust = np.nanmean(np.nanmean(stacked_dust,axis=0),axis=0)


stacked_ssa = np.stack([aod_cams_ssa0,aod_cams_ssa0,aod_cams_ssa0,aod_cams_ssa0],axis=0)
aod_cams_ssa = np.nanmean(np.nanmean(stacked_ssa,axis=0),axis=0)
stacked_dust = np.stack([aod_cams_dust0,aod_cams_dust0,aod_cams_dust0,aod_cams_dust0],axis=0)
aod_cams_dust = np.nanmean(np.nanmean(stacked_dust,axis=0),axis=0)

print(aod_cams_ssa[aod_cams_ssa>0])

print('aod_cams_ssa.shape',aod_cams_ssa.shape)
lat_bins = np.arange(-90.1, 90.1, 2)
lon_bins = np.arange(-180.1, 180.1, 2)

lat_cams_0 = fcams0['latitude'].values
lon_cams_0 = fcams0['longitude'].values
...
faods = ['2025_april_aod_ssa.txt','2025_april_aod_dust_alike.txt']
...
for aero_type,fig_name, faod,caod in zip(aero_types,fig_names,faods,caods):
    ...
    print('CAMS AOD >0 mean=',np.nanmean(caod[aod_atlid>0]))
    print('ATLID AOD >0 mean=',np.nanmean(aod_atlid[aod_atlid>0]))
    print('CAMS-ATLID >0 mean=',np.nanmean(caod[aod_atlid>0]-aod_atlid[aod_atlid>0]))
   
    print('CAMS AOD mean=',np.nanmean(caod[caod>-9]))
    print('ATLID AOD mean=',np.nanmean(aod_atlid[caod>-9]))
    print('CAMS-ATLID mean=',np.nanmean(caod[caod>-9]-aod_atlid[caod>-9]))
    ...
    im=ax3.pcolormesh(clons,clats,caod-aod_atlid,cmap='RdBu_r',transform=ccrs.PlateCarree(),norm=colors.SymLogNorm(linthresh=0.01,linscale=1.0,vmax=vmax,vmin=-vmax))
    ...
    plt.tight_layout()
    fig.savefig('global_aod_'+aero_type+'_2deg_binned_'+mean_or_std+'_april_2025_trimedges_filterCAMScloud_TC_TTcal.jpg',bbox_inches='tight')
   
    nbins = 150
    binsc = np.linspace(0,caod.max(),nbins)
    histc,binsc = np.histogram(caod,bins=binsc,density=False)
    ...
    fig.savefig('global_'+aero_type+'_histograms_CAMS_ATLID_2deg_binned_'+mean_or_std+'_April_2025_trimedges_filterCAMScloud_TC_TTcal.jpg')
