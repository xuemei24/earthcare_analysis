...
cams_dir2 = '/net/pc200254/nobackup/users/wangxu/cams_data/aerosol_mmr/'+month+'/'
...
for lforecast_period in forecast_periods:
    ...
    for iff in range(length): #forecast reference time
        ...
        ftemp = xr.open_dataset(cams_dir+'temperature/temperature_'+month+'_2025_'+lforecast_period+'.nc') #xxx #1,60,137,451,900
        Temperature_f = ftemp['t'][0,iff]#xxx
        ftemp.close()
 
        fqt = xr.open_dataset(cams_dir+'qt/specific_humidity_'+month+'_2025_'+lforecast_period+'.nc') #xxx #1,60,137,451,900
        ...
        for w in [10,11]: # 10 = 393 nm, 11 = 304 nm
            for i in range(len(MR_names)):  # Loop through tracer names
                file_path = cams_dir2+'cams_original_aerosol_mmr/'+cams_data[i]+suffix1+lforecast_period+suffix2
    ...
    filepath = os.path.join(cams_dir2+'/TTcal/',
