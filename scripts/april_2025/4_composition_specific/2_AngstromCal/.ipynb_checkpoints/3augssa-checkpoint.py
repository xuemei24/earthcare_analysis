...
if which_aerosol == 'dust_om':
    fname = 'dust_om'

cams_dir = '/scratch/nld6854/earthcare/cams_data/'+month+'_2025/TTcal/'
fcams = cams_dir+'Angstrom_aod355nm_per_composition_'+month+"_2025.nc"
...
if which_aerosol == 'ssa':
    cams_aod = ds['ssa_aod_550_AE_550to355'].values[:,:,::3]
elif which_aerosol == 'dust':
    cams_aod = ds['du_aod_550_AE_550to355'].values
elif which_aerosol == 'dust_om':
    cams_aod = ds['du_om_aod_AE_550to355'].values[:,:,::3]
...
print('lwc_final.shape',lwc_final.shape,'cams_aod.shape',cams_aod.shape)
...
def process_file(filen):
    ...
    atlid_aod, atlid_lats, atlid_lons, atlid_times = read_h5.get_aod(filen, np.array(tcs))
...
# run in parallel
ebd_files = sorted(glob.glob(srcdir+'*h5'))[:10]
...
# save once
np.savetxt(cams_dir+'AngstromCal_'+fname+'_aod355nm_per_composition_'+month+'_2025_cams_atlid_co-located_3hourly.txt',
