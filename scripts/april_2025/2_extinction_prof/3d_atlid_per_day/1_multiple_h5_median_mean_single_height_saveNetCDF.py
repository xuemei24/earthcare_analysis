...
month = 'october'#'november'#'august'#'september'#'december'
month2 = '10'
no_of_days = 31
year = '2024'
...
for nday in range(1,no_of_days+1):
    nday = str(nday) if nday>=10 else '0'+str(nday)
    file_paths = glob.glob('/scratch/nld6854/earthcare/earthcare_data/'+month+'_'+year+'/EBD/'+'*_ATL_EBD_2A_'+year+month2+nday+'*.h5')
    print(file_paths)
    file_paths.sort()
    ...
    outdir = f"/scratch/nld6854/earthcare/cams_data/{month}_{year}/daily"
    os.makedirs(outdir, exist_ok=True)
    
    outfile_normal = (
        f"{outdir}/regridded_satellite_{which_aerosol}"
        f"_extinction_coe_2deg_{mean_or_std}_single_alt_{year}_{month}_{nday}_snr_gr_2.nc"
    )
    
    outfile_masknan = (
        f"{outdir}/regridded_satellite_{which_aerosol}"
        f"_extinction_coe_2deg_masknan_{mean_or_std}_single_alt_{year}_{month}_{nday}_snr_gr_2.nc"
    )
