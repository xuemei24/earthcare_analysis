...
all_height_padded = [np.pad(arr, ((0, 0), (0, max_cols - arr.shape[1])), mode='constant')
    if arr.shape[1] < max_cols else arr
    for arr in all_height]

print('all_height_padded[0]',all_height_padded[0].shape)
regridded_data = np.zeros((len(lat_bins)-1,len(lon_bins)-1, max_cols))
regridded_height = np.zeros((len(lat_bins)-1,len(lon_bins)-1, max_cols))
...
print('regridding data finished')
for i in range(max_cols):
    all_height1 = [all_height_ij[:,i] for all_height_ij in all_height_padded]
    all_heightw_padded = [pad_array(a, max_height) for a in all_height1]
    all_height = np.concatenate(all_heightw_padded, axis=0)

   
    # Compute 2D histogram for the mean wind
    stat, x_edge, y_edge, _ = binned_statistic_2d(
        all_lat, all_lon, all_height, statistic='mean', bins=[lat_bins, lon_bins])
...
print('regridding height finished')
...
output_filename = "regridded_satellite_"+which_aerosol+"_extinction_coe_2deg_mean_all_alt_april_2025.nc"
...
regridded_data = np.zeros((clons.shape[0],clons.shape[1], max_cols))
regridded_height = np.zeros((clons.shape[0],clons.shape[1], max_cols))
...
for i in range(max_cols):
    ...
    mask = ~np.isnan(all_height)
    all_height = all_height[mask]
    all_lat2 = all_lat[mask]
    all_lon2 = all_lon[mask]

    # Compute 2D histogram for the mean wind
    stat, x_edge, y_edge, _ = binned_statistic_2d(
        all_lat2, all_lon2, all_height, statistic='mean', bins=[lat_bins, lon_bins])
...
# Save the regridded data to a NetCDF file
output_filename = "regridded_satellite_"+which_aerosol+"_extinction_coe_2deg_masknan_mean_all_alt_april_2025.nc"
regridded_data_xr.to_netcdf(output_filename)
