...
def get_ext_wrapper(args):
    return read_h5.get_ext_col(*args)
...
for month in months:
    ...
    n_height = height_list[0][0,:].shape[0]
    ...
    print("Finished regridding: Version WITH masknan, min_count",min_count)
    ...
    outfile_normal = (
        ...
        f"_extinction_coe_2deg_{mean_or_std}_single_alt_{month}_{year}_snr_gr_2_npnan.nc"
    )
    
    outfile_masknan = (
        f"{outdir}/regridded_satellite_{which_aerosol}"
        f"_extinction_coe_2deg_masknan_{mean_or_std}_single_alt_{month}_{year}_snr_gr_2_npnan.nc"
    )
