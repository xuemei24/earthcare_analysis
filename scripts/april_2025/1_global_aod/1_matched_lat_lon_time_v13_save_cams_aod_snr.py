...
def process_file(filen):
    ...
    return (atlid_lats, atlid_lons, atlid_aod, cams_aod, atlid_times)
...
# combine results
all_lat, all_lon, a_aod, c_aod, a_time = [], [], [], [], []
for res in results:
    if res is None:
        continue
    lat, lon, aod, caod, atime = res
    all_lat.extend(lat)
    all_lon.extend(lon)
    a_aod.extend(aod)
    c_aod.extend(caod)
...
np.savetxt(cams_dir+year+'_'+month+'_cams_atlid_co-located_'+day_or_night+'aod_snr_gr_2.txt',
           np.array([all_lat, all_lon, a_time, a_aod, c_aod]).T,
           header='latitude,longitude,time,aod355nm_atlid,aod355nm_cams',
           delimiter=',')
