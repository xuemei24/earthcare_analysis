...
df = pd.read_csv("../2025_"+month+"_aod.txt", delimiter=",")
...
vmax = 1
print('aod_tropomi.max()=',aod_tropomi.max())
...
print('TROPOMI AOD >0 mean=',np.nanmean(aod_tropomi[aod_atlid>0]))
print('ATLID AOD >0 mean=',np.nanmean(aod_atlid[aod_atlid>0]))
print('TROPOMI-ATLID >0 mean=',np.nanmean(aod_tropomi[aod_atlid>0]-aod_atlid[aod_atlid>0]))

print('TROPOMI AOD mean=',np.nanmean(aod_tropomi[aod_atlid>-9]))
print('ATLID AOD mean=',np.nanmean(aod_atlid[aod_atlid>-9]))
print('TROPOMI-ATLID mean=',np.nanmean(aod_tropomi[aod_atlid>-9]-aod_atlid[aod_atlid>-9]))
print('TROPOMI-ATLID mean=',np.nanmean(aod_tropomi[aod_tropomi>-9]-aod_atlid[aod_tropomi>-9]))
print('ATLID,TROPOMI NMB=',statistics.normalized_mean_bias(aod_atlid,aod_tropomi))
...
ax1.set_title('ATLID integrated AOD '+month,fontsize=15)



ax2.set_extent([-180, 180, -89.9, 89.9])
im=ax2.pcolormesh(clons,clats,aod_tropomi,cmap='plasma',transform=ccrs.PlateCarree(),norm=colors.LogNorm(vmin=1e-2, vmax=1))
...
bar.ax.tick_params(labelsize=15)
ax2.set_title('TROPOMI total AOD',fontsize=15)

ax3.set_extent([-180, 180, -89.9, 89.9])
im=ax3.pcolormesh(clons,clats,aod_tropomi-aod_atlid,cmap='RdBu_r',transform=ccrs.PlateCarree(),vmax=vmax,vmin=-vmax)
...
bar.ax.set_ylabel('TROPOMI-ATLID Particle optical depth / -',fontsize=15)
bar.ax.tick_params(labelsize=15)
ax3.set_title('TROPOMI-ATLID AOD',fontsize=15)

plt.tight_layout()
fig.savefig('global_aod_'+which_aerosol+'_2deg_binned_'+mean_or_std+'_'+month+'_2025_trimedges_TROPOMI_TC.jpg',bbox_inches='tight')

nbins = 150
aod_tropomi_clean = aod_tropomi[~np.isnan(aod_tropomi)]
binsc = np.linspace(0,np.nanmax(aod_tropomi),nbins)
histc,binsc = np.histogram(aod_tropomi_clean,bins=binsc,density=False)
...
ax1.plot(bcc,histc,label='TROPOMI')
...
fig.savefig('histograms_TROPOMI_ATLID_2deg_binned_'+mean_or_std+'_'+month+'_2025_trimedges_TC.jpg')
