...
def regional_mean_aod_weighted_np(aod, lat, lon,
                                  landsea_mask=None, land_only=False, sea_only=False):
    """
    aod:       (lat, lon) numpy array
    lat:       (lat,)
    lon:       (lon,)
    landsea_mask: (lat, lon) array with 1 = land, 0 = sea  (optional)
    """

    aod_sel = aod   # (nlat, nlon)
    lat_sel = lat
    lon_sel = lon

    # Optional land/sea restriction
    if landsea_mask is not None:
        lsm_sel = landsea_mask[lat_mask][:, lon_mask]   # shape (nlat, nlon)

        if land_only:
            aod_sel = np.where(lsm_sel == 1, aod_sel, np.nan)

        if sea_only:
            aod_sel = np.where(lsm_sel == 0, aod_sel, np.nan)

    # Latitude-based area weights: cos(lat)
    weights_lat = np.cos(np.deg2rad(lat_sel))   # (nlat,)
    weights_2d  = weights_lat[:, None] * np.ones(len(lon_sel))[None, :]

    # Weighted mean
    numerator   = np.nansum(aod_sel * weights_2d)
    denominator = np.nansum(weights_2d)
