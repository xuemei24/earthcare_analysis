...
itlon = np.where(t_lon>=180)
t_lon[itlon] = t_lon[itlon]-360
...
aod_cams0 = np.nanmean(where_cloudy(clwc0.variables['clwc'][0,1:],aod_o_cams[0]),axis=0)
aod_cams3 = np.nanmean(where_cloudy(clwc3.variables['clwc'][0,1:],aod_o_cams[1]),axis=0)
aod_cams6 = np.nanmean(where_cloudy(clwc6.variables['clwc'][0,1:],aod_o_cams[2]),axis=0)
aod_cams9 = np.nanmean(where_cloudy(clwc9.variables['clwc'][0,1:],aod_o_cams[3]),axis=0)
...
plt.tight_layout()
fig.savefig('global_aod_'+which_aerosol+'_2deg_binned_'+mean_or_std+'_'+month+'_2025_trimedges_filterCAMScloud_TC.jpg',bbox_inches='tight')
...
fig.savefig('histograms_CAMS_ATLID_2deg_binned_'+mean_or_std+'_'+month+'_2025_trimedges_filterCAMScloud_TC.jpg')
