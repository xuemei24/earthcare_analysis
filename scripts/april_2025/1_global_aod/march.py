...
def get_ssa_aod_wrapper(args):
    return read_h5.get_aod2(*args)

# List of file paths you want to read concurrently
file_paths = glob.glob('/net/pc190625/nobackup/users/wangxu/earthcare_data/march_2025/EBD/*.h5')
file_paths.sort()
...
np.savetxt('2025_march_aod.txt',np.array([all_lat,all_lon,all_aod]).transpose(),header='latitude,longitude,aod355nm',delimiter=',')
