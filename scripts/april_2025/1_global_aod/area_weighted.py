...
def regional_mean_aod_weighted_np(aod, lat, lon,
    ...
    # Optional land/sea masking
    if landsea_mask is not None:
        if land_only:
            aod_sel = np.where(landsea_mask == 1, aod_sel, np.nan)
        if sea_only:
            aod_sel = np.where(landsea_mask == 0, aod_sel, np.nan)

    # Latitude-based weights (cos(lat))
    weights_lat = np.cos(np.deg2rad(lat))             # (nlat,)
    weights_2d  = weights_lat[:, None] * np.ones(len(lon))[None, :]
