...
aod_atlid= np.where(aod_atlid<=0.032,aod_atlid,np.nan)
...
#aod_cams= np.where(((aod_atlid>=0.032) & (aod_atlid<=0.05)),aod_cams,np.nan)
aod_cams= np.where(aod_atlid<=0.032,aod_cams,np.nan)
...
plt.tight_layout()
fig.savefig('figures/global_'+day_or_night+'aod_'+which_aerosol+'_'+str(reso)+'deg_binned_'+mean_or_std+'_'+month+'_'+year+'_co-located_filtered_uncertainty_where_are_aerosols.jpg',bbox_inches='tight')
