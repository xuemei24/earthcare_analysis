...
def ecplt_cmap(ds,varname,line_break = None):
    cleanup_category = lambda s: s.strip().replace('possible', 'poss.').replace('supercooled', "s\'cooled").replace("stratospheric", 'strat.').     replace('extinguished', 'ext.').replace('precipitation', 'precip.').replace('and', '&').replace('unknown', 'unk.').replace('precipitation', 'pr     ecip.')                            
    ...
    if "\n" in ds[varname].attrs['definition']:
        ...
        categories = [cleanup_category(s) for s in ds[varname].attrs['definition'].replace("ground clutter,", "ground clutter;").replace('va     lid, quality','valid; quality').replace('valid, degraded','valid; degraded').split(',')]
